  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.galleryItems && nextProps.biddingItems &&
      ...
      if (nextProps.galleryItems.length > 0) {
        ...
        const highleBiddedItems = sortedGalleryItems.filter(x => highBiddedItemIds.has(get(x, 'id')));
        const remainingItems = sortedGalleryItems.filter(x => sortedGalleryItems.has(get(x, 'id')));
        ...
        **/
        ...
        const state = Object.assign({}, this.state, { highleBiddedItems, remainingItems, loading: false });
        this.setState(state);
      }
    }
  }
            ...
            <div className="img-thumbnails">
              {
                this.props.galleryItems.map(item =>
                  <Lightbox>
                    <LightboxTrigger >
                      <a onClick={() => this.updateMainItem(item)} className="galleryCont1">
